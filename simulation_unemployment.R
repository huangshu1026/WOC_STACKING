...
n.test <- 1000
draws <- 100
#rmse.test  <- array(NA, dim = c(length(n.vec.train), draws, 11+2)) # 11 component weighting methods + our stacking method + vanilla stacking method 
for(j in 1:length(n.vec.train)){
  ...
  for(k in 1:draws){
    ...
    write.table(t(c(n.vec.train[j], k, cor_vec)), "sim_unemp_corr.csv", row.names = FALSE, col.names = FALSE, append = TRUE, sep = ",")
    write.table(t(c(n.vec.train[j], k, sqrt(colMeans((all.pred - Y.test)^2)))), "sim_unemp_rmse.csv", row.names = FALSE, append = TRUE, col.names = FALSE, sep = ",")
    write.table(t(c(n.vec.train[j], k, coef.stacking)), "sim_unemp_coef.csv", row.names = FALSE, append = TRUE, col.names = FALSE, sep = ",")
  }
}
